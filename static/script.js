...
function toggleDropdown(id) {
    var dropdown = document.getElementById(id.replace("dd", "dropdown"));
...
function appendPostConfig() {
    const configBtn = document.createElement("div");
    configBtn.className = "dropdown";
    // Add the button and dropdown content to the dropdown element
    configBtn.innerHTML = `
    <button class="dots-button" onclick="toggleDropdown(this.id)">...</button>
    <div class="dropdown-content">
        <a onclick="editPost(this.id)">Edit</a>
        <a onclick="postHandler(this.id)">Delete</a>
    </div>
    `;    
    console.log(document.querySelectorAll('.posts'))
    let postDivs = document.querySelectorAll('.posts')
    for (let i = 0; i < postDivs.length; i++ ) {
        console.log(postDivs[i].querySelector('pre').textContent, accountLink.textContent)
        let author = postDivs[i].querySelector('pre').textContent
        if (author !== accountLink.textContent) {
            continue
        }
        configBtn.querySelector('button').id = postDivs[i].id.replace("post", "dd") 
        configBtn.querySelector('div').id = postDivs[i].id.replace("post", "dropdown")
        configBtn.querySelectorAll('a').forEach((anchor)=> {
            console.log(anchor.textContent)
            switch (anchor.textContent) {
                case "Edit":
                anchor.id = postDivs[i].id.replace("post", "edit")
                break
                case "Delete": 
                anchor.id = postDivs[i].id.replace("post", "delete")
                break
            }
            console.log(anchor)
        })
        console.log(configBtn) 
        postDivs[i].insertBefore(configBtn.cloneNode(true), postDivs[i].firstChild)
    }
...
const postElement = document.querySelector(`.post[data-post-id="${postId}"]`);
  ...
  // Get the post's title and content
...
editForm.innerHTML = `
    <input type="text" name="newTitle" value="${title}" placeholder="New Title">
    <textarea name="newContent" placeholder="New Content">${content}</textarea>
    <button type="submit">Save</button>
`;

  // Replace the post's content with the editing form
postElement.innerHTML = '';
postElement.appendChild(editForm);

  // Add a submit event listener to the form to handle the update
editForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const newTitle = editForm.querySelector('input[name="newTitle"]').value;
    const newContent = editForm.querySelector('textarea[name="newContent"]').value;
    postHandler('edit', postId, newTitle, newContent)
});
}

function postHandler(action, postId, title, content) {
    ...
    fetch("/post-handler", {
        ...
        body: JSON.stringify({id:postId, title: title, content: content, username: action})
    });
