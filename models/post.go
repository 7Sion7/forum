...
func SavePost(title, content string, image multipart.File, userId int) (int, error) {
	...
	result, err := db.Exec("INSERT INTO posts (title, content, image, userId) Values (?, ?, ?, ?)", title, content, imageData, userId)
	if err != nil {
...
func GetPostsFromDB(userID int) ([]Post, error) {
	query := `
        SELECT 
            posts.id, posts.title, posts.content, posts.image, users.username,
	...
	for rows.Next() {
		...
		err := rows.Scan(&postID, &title, &content, &imageData, &username, &categoryNames, &likes, &dislikes, &userLikeStatus)
		if err != nil {
			return nil, err
		}
